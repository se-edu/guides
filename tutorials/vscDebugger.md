...
**Common Issues Faced**
1. **Debugger won't start**, debugger process terminated abruptly
  *  Ensure JDK is installed (`java --version` and `javac --version`)
  *  Verify `launch.json`: Correct mainClass path e.g. 'com.example.Main'
2. **Breakpoints not working**, code runs but does not pause at breakpoints
...
3. **Class not found** or Error: "Could not find or load main class"
