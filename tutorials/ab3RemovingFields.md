{% set title="AB3 Tutorial: Removing Fields" %}
---
title: "{{ title }}"
pageNav: 3
---
...
**If you have done the [Add `remark` command tutorial](ab3AddRemark.html)  already**, you should know where the code had to be updated to add the field `remark`. From that experience, you can deduce where the code needs to be changed to _remove_ that field too. The removing of the `address` field can be done similarly.
<br>
<br>
...
<pic src="images/remove/UnsafeDelete.png" />
...
<pic src="images/remove/SafeDeleteConflicts.png" />

Remove usages of `Address` by performing `Safe Delete`s on each entry, i.e., double-click the entry (which takes you to the relevant code), right-click that entity, and choose `Refactor` -> `Safe delete` as before. You will need to exercise discretion when removing usages of `Address`. Functions like `ParserUtil#parseAddress()` can be safely removed but its usages must be removed as well. Other usages like in `EditPersonDescriptor` may require more careful inspection.
...
1. Select `View Usages` again.<br>
   <pic src="images/remove/UnsafeDeleteOnField.png"/>
...
<pic src="images/remove/$address.png" width="800" />
...
After you've manually removed the address field from each relevant JSON file, you can cross-check your changes in this [commit](https://github.com/se-edu/addressbook-level3/commit/3e658057198ca9e1ff0fce2ce1b353df78e0be86).
